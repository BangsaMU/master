...
@section('header')
    <h1 class="page__title font-bold text-2xl m-0">
        {{ $param ? 'Edit' : 'Create' }} Unit of Measurement
    </h1>
                ...
                <div class="card__body p-6">
                    <form action="{{ $data['page']['store'] }}" method="POST" autocomplete="off" class="flex flex-col gap-4">
                        ...
                        <div class="field flex flex-col gap-1">
                            <label class="field__label text-sm font-medium text-foreground" for="uom_code">UOM Code</label>
                            <input {{ $data['page']['readonly'] ? 'readonly' : '' }} type="text" name="uom_code"
                                id="uom_code" class="input w-full @error('uom_code') border-danger @enderror"
                        ...
                        <div class="field flex flex-col gap-1">
                            <label class="field__label text-sm font-medium text-foreground" for="uom_name">UOM Name</label>
                            <input {{ $data['page']['readonly'] ? 'readonly' : '' }} type="text" name="uom_name"
                                id="uom_name" class="input w-full @error('uom_name') border-danger @enderror"
                        ...
                        <div class="flex items-center gap-2 mt-4 pt-4 border-t border-border">
                            ...
                            @endif
                            <a href="{{ Route::has('master.uom.index') ? route('master.uom.index') : '#' }}" class="button button--neutral button--outline">
                                Back
                            </a>
                        </div>
                    </form>
